...
//REACT & REDUX LIBRARIES SET UP FOR CODEPEN
// const { Component } = React;
// const { update } = React.addons; //DEPRECATED
...
//a redux action creator function used to return an action.
...
//function that takes a grid array, an object and default value for range.
const createRoomsFromSeed = (grid, {x, y, width, height}, range = c.ROOM_SIZE_RANGE) => {
  ...
  const roomValues = [];
//take 4 directions(N,E,S,W), place one room on each object given to it,
//then check if each room can be placed inside our grid.
//If room can be placed in grid...refer to placeCells();
  ...
  //we need the last room of the array for generating other rooms in 4 directions
...
//function parameters; grid array, array to store rooms, 2 integers(counter and maxRooms);
